...
@registry.register_measure
class PL(Measure):
    """Official FutureNav DAgger path-efficiency ratio.

    This is not the same as the `path_length` measurement.  The DAgger save rule
    uses this value as `metrics["pl"]`:
      start_end_episode_distance / max(start_end_episode_distance, path_length_so_far)
    """
